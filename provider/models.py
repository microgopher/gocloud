...
class ProviderModel(Base):
    ...
    specializations = db.Column(db.String(256), nullable=True)
    practicing_start_from = db.Column(db.DateTime, nullable=True)
    photo = db.Column(db.LargeBinary, nullable=True)
    user_id = db.Column(db.String(20), nullable=False) #user's hex_id

class QualificationModel(Base):
    __tablename__ = 'qualification'
    institution_name = db.Column(db.String(120), nullable = True)
    procurement_year = db.Column(db.Integer)
    provider_id = db.Column(db.Integer, db.ForeignKey('providers.id'), nullable=False)

class AffiliationModel(Base):
